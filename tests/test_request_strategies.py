...
def request_strategy_factory(
    strategy_class, build_client_with="json", **kwargs
):
...
def assert_mock_client_called_once(mock_client, expected_request_data):
    ...
    assert (
        mock_client.client.get_default_username_password_authentication.call_count
        == 1
    )
    assert mock_client.request_formatter.format.call_count == 1
    assert mock_client.request_formatter.format.call_args == call(
...
def test_request_strategy_get_method_delegates_to_parent_handlers(
    mock_requests, mock_client
):
    mock_requests.get(
        "mock://testserver.com", json={"active": True}, status_code=200
    )

    strategy = RequestStrategy()
    strategy.set_client(mock_client.client)

    response = strategy.get(
        "mock://testserver.com", params={"foo": sentinel.params}
    )
...
def test_request_strategy_post_method_delegates_to_parent_handlers(
    mock_requests, mock_client
):
    mock_requests.post(
        "mock://testserver.com", json={"active": True}, status_code=200
    ...
    response = strategy.post(
        "mock://testserver.com",
        data={"data": sentinel.data},
        params={"foo": sentinel.params},
    )
...
def test_request_strategy_patch_method_delegates_to_parent_handlers(
    mock_requests, mock_client
):
    mock_requests.patch(
        "mock://testserver.com", json={"active": True}, status_code=200
...
def test_request_strategy_delete_method_delegates_to_parent_handlers(
    mock_requests, mock_client
):
    ...
    response = strategy.delete(
        "mock://testserver.com", params={"foo": sentinel.params}
    )

    assert response == sentinel.result
    assert_request_called_once(
        mock_requests, "mock://testserver.com", "DELETE"
    )
    assert_mock_client_called_once(mock_client, None)


@pytest.mark.parametrize("initial_params", [{"my-param": "always-set"}, None])
def test_query_param_paginated_strategy_delegates_to_callable(
    initial_params, mock_requests
):
    ...
    mock_requests.get(
        ...
        [
            {
                "json": {"data": ["element1", "element2"], "nextPage": "2"},
                "status_code": 200,
            },
            {
                "json": {"data": ["element3", "element4"], "nextPage": None},
                "status_code": 200,
            },
        ],
    )
    ...
    strategy = request_strategy_factory(
        QueryParamPaginatedRequestStrategy, next_page=next_page_callback
    )
...
def test_url_paginated_strategy_delegates_to_callable(mock_requests):
    ...
    mock_requests.get(
        ...
        json={
            ...
            "nextPage": "mock://testserver.com/2",
        },
        status_code=200,
    )
    mock_requests.get(
        "mock://testserver.com/2",
        json={"data": ["element3", "element4"], "nextPage": None},
        status_code=200,
    )
    ...
    strategy = request_strategy_factory(
        UrlPaginatedRequestStrategy, next_page=next_page_callback
    )
