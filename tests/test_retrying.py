...
def testing_retries(max_attempts=None, wait=None):
    ...
    if max_attempts is not None:
        _retry_enabled_function.retry.stop = tenacity.stop_after_attempt(
            max_attempts
        )
    if wait is not None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            make_retry_state(
                previous_attempt_number=1, delay_since_first_attempt=0
            ),
            0.25,
        ),
        (
            make_retry_state(
                previous_attempt_number=2, delay_since_first_attempt=0
            ),
            0.5,
        ),
        (
            make_retry_state(
                previous_attempt_number=3, delay_since_first_attempt=0
            ),
            1,
        ),
        (
            make_retry_state(
                previous_attempt_number=4, delay_since_first_attempt=0
            ),
            2,
        ),
        (
            make_retry_state(
                previous_attempt_number=5, delay_since_first_attempt=0
            ),
            4,
        ),
        (
            make_retry_state(
                previous_attempt_number=6, delay_since_first_attempt=0
            ),
            8,
        ),
        (
            make_retry_state(
                previous_attempt_number=7, delay_since_first_attempt=0
            ),
            16,
        ),
        (
            make_retry_state(
                previous_attempt_number=8, delay_since_first_attempt=0
            ),
            30,
        ),
    ],
...
def test_exponential_retry_backoff_not_greater_than_30s(
...
def test_retries_for_status_codes_over_5xx(status_code):
    side_effects = (
        APIRequestError("Something went wrong.", status_code),
        sentinel.result,
    )
    callable = RunnableCounter(side_effects)
    with testing_retries(max_attempts=5, wait=0) as func:
